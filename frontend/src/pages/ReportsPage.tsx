    ...
    <div className="min-h-full bg-gray-50 dark:bg-gray-950">
      <div className="border-b border-gray-200 bg-white px-6 py-4 dark:border-gray-800 dark:bg-gray-900">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-lg font-bold text-gray-900 dark:text-gray-100">Reportes</h1>
            <p className="text-sm text-gray-500 mt-0.5 dark:text-gray-400">Métricas de prospección y pipeline</p>
          </div>
          ...
          <>
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <h2 className="text-sm font-semibold text-gray-900 mb-4 dark:text-gray-100">Distribución por estado</h2>
              ...
              {/* Barras por estado */}
              <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900">
                <h2 className="text-sm font-semibold text-gray-900 mb-4 dark:text-gray-100">Empresas por estado</h2>
                  ...
                  {STAGES.map((stage) => {
                    ...
                    return (
                      <div key={stage} className="flex flex-1 flex-col items-center gap-2">
                        ...
                        <span className="text-[10px] text-gray-500 text-center dark:text-gray-400">{stage}</span>
                      </div>
                    );
                  })}
                </div>
            ...
            <div className="rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
              <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-800">
                <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Ranking por vendedora</h2>
              </div>
              {assignedStats.length === 0 ? (
                <div className="py-10 text-center text-sm text-gray-400 dark:text-gray-500">Sin datos</div>
              ) : (
                <div className="divide-y divide-gray-100 dark:divide-gray-800">
                  {assignedStats.map(([name, count], idx) => (
                    <div key={name} className="flex items-center gap-3 px-5 py-3">
                      ...
                      <span className="flex-1 text-sm font-semibold text-gray-900 dark:text-gray-100">{name}</span>
                      <span className="text-sm text-gray-500 dark:text-gray-400">{count} empresa{count !== 1 ? "s" : ""}</span>
                      <div className="w-32 h-2 rounded-full bg-gray-100 overflow-hidden dark:bg-gray-800">
            ...
            <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-x-auto dark:border-gray-800 dark:bg-gray-900">
              <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-800">
                <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Todas las empresas ({companies.length})</h2>
              </div>
                ...
                <tbody>
                  {companies.slice(0, 10).map((c: Company) => (
                    <tr key={c.id} className="border-t border-gray-100 hover:bg-gray-50 transition-colors dark:border-gray-800 dark:hover:bg-gray-800/60">
                      <td className="px-5 py-3 text-sm font-semibold text-gray-900 dark:text-gray-100">{c.name}</td>
                      ...
                      <td className="px-5 py-3 text-sm text-gray-400 dark:text-gray-500">{new Date(c.createdAt).toLocaleDateString("es-CO")}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              {companies.length > 10 && (
                <div className="px-5 py-3 text-center border-t border-gray-100 dark:border-gray-800">
                  <p className="text-xs text-gray-400 dark:text-gray-500">Mostrando 10 de {companies.length} · Usa "Exportar CSV" para ver todas</p>
                </div>
              )}
            </div>
          </>
